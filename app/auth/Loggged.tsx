...
export default function Logged({image}:User) {
  ...
  return (
    <div className="flex lg:gap-8 gap-2">
      <select onChange={handleGroupSelect} className="block appearance-none md:hidden  bg-blue-500  text-white p-2 rounded-xl leading-tight focus:outline-none focus:bg-blue-600 focus:border-gray-500 text-center">
        <option value="" >
          Pick a group
        </option>
      ...
      <Link href={"/dashboard"}>
        <Image width={64} height={64} src={image} className="w-12 lg:w-14 rounded-full" alt="" priority />
      </Link>
    </div>
  );
}
